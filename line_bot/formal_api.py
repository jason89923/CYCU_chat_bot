...
gv.LB_INFO_CACHE = redis.Redis(host='localhost', port=6379, decode_responses=True, db=3)
# gv.LB_INFO_CACHE.flushdb()
...
class API:
    ...
    def get_lb_info(opcode, year):
        ...
        if gv.LB_INFO_CACHE.exists(primary_key):
            ...
            if 'dataList' not in response:
                gv.LB_INFO_CACHE.set(primary_key, json.dumps(None), ex=300)
                return None
            for i in response['dataList']:
                ...
                if 'webpac_info' in i:
                    for j in i['webpac_info'][0]:
                        if j['title'] in gv.LB_INFO_MAP:
                            info_list[gv.LB_INFO_MAP[j['title']]] = j['content']
                if info_list != {}:
                    result.append(info_list)
            if len(result) == 0:
                result = None
            gv.LB_INFO_CACHE.set(primary_key, json.dumps(result), ex=300)
            return result
    ...
    def itouch_login(uid):
        ...
        url = LOGIN_URL + f"/active_system/line/bind.jsp?line_uid={uid}&path={gv.URL}successful_login.html/&isReturn=0"
...
def API_test():
    #pprint(API.get_op_class('1121'))

    # pprint(API.get_st_info('10811245'))  # 個人資訊 : 推薦
    # pprint(API.get_lb_info('ID347D', '1112'))
    # pprint(API.get_std_course_info('11015247')) # 修課核對表 : 畢業門檻
    # pprint(API.itouch_login('U55e6eea5b53b07e974f06f8cf8b2f2f7'))
    # pprint(API.get_std_id('U85b23a2893e94d12a16721ade9118bab')) 
    # pprint(API.add_idno('U824cac8fefdf7daf96bf16832667349d','G122476952'))
    # pprint(API.del_acct('U55e6eea5b53b07e974f06f8cf8b2f2f7'))
    pprint(API.course_list('11120304'))
